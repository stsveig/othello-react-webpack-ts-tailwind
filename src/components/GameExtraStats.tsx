...
export const GameExtraStats: React.FunctionComponent = () => {
  const { game, resetGame, passMyTurn } = useOthelloGameState();

  return (
    <>
      <div className="col-span-2 bg-[#303030] py-3 flex flex-col items-center rounded-md h-32 relative select-none">
        <div className="flex">
          ...
          <button
            type="button"
            onClick={() => passMyTurn()}
            className={`${game.vanillaOthello ? "hidden" : ""} ${basicStyle}`}
          >
            Pass
          </button>
          <button type="button" className={basicStyle}>
